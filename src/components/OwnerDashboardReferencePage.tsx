...
import { getFallbackMarketplaceMapProperties, parseMarketplacePrice } from './MarketplaceReferencePage';
...
function mapAvailableListing(row: OwnerListingRow): Property | null {
  ...
  return {
    ...
    agent: { id: 'flx-owner-map', name: 'FLX Real Estate', avatar: '', phone: '', email: '', license: 'FLX', role: 'Agent' },
...
export function OwnerDashboardReferencePage() {
  ...
  return <ProtectedWorkspaceShell section="Owner dashboard" location="Dar es Salaam" backHref="/marketplace"><div className="owner-reference-page">
    <aside className={`owner-reference-sidebar ${mobileNav ? 'is-open' : ''}`}><div className="owner-reference-logo" role="img" aria-label="FLX Real Estate, Deal OS Tanzania" title="FLX Real Estate"><span className="owner-reference-logo-full">FLX Real Estate</span><span className="owner-reference-logo-mark" aria-hidden="true">FLX</span><small>Deal OS Tanzania</small></div><div className="owner-reference-portfolio" aria-label="Haji Properties Ltd, Mwenge and Posta Portfolio" title="Haji Properties Ltd · Mwenge & Posta Portfolio"><i /> <span><b>Haji Properties Ltd</b><small>Mwenge & Posta Portfolio</small></span><ChevronDown size={14} /></div><small className="owner-reference-nav-label">Enterprise Navigation</small><nav>{[['Dashboard / Overview', Activity], ['Deal Pipeline', ArrowRight], ['Property Portfolio & Units', Building2], ['Financials & Escrow Ledger', WalletCards], ['Maintenance & Fundis', Wrench], ['Contracts (Mkataba Pro)', FileCheck2], ['System Settings', Settings]].map(([label, Icon]) => <button key={String(label)} className={label === activeWorkspace ? 'is-active' : ''} type="button" aria-label={String(label)} title={String(label)} onClick={() => { setActiveWorkspace(String(label)); setMobileNav(false); }}><Icon size={17} /><span>{label}</span></button>)}</nav><div className="owner-reference-node" aria-label="Node status: TRA and Ardhi Mesh Sync" title="TRA & Ardhi Mesh Sync"><span>NODE STATUS <i /></span><b>TRA & Ardhi Mesh Sync</b><small>Dar es Salaam Tier-4 Gateway</small></div></aside>
    <div className="owner-reference-shell"><header className="owner-reference-header"><button className="owner-reference-menu" type="button" aria-label="Open navigation" onClick={() => setMobileNav((open) => !open)}>{mobileNav ? <X size={20} /> : <Menu size={20} />}</button><div className="owner-reference-place"><MapPinIcon /> Dar es Salaam, TZ</div><div className="owner-reference-search"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search deals, title deeds, Masaki / Upanga units, tenants..." /></div><button className="owner-reference-add" type="button" onClick={() => setUnitModal(true)}><Plus size={16} /> Add Unit / Listing</button><button className="owner-reference-notify" type="button" aria-label="Notifications"><Bell size={18} /></button></header>
